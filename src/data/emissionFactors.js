...
export const EMISSION_FACTORS = {
  ...
  transport: {
    car: {
      petrol: 0.17, // Average petrol/gasoline car, medium size — DEFRA 2023
      diesel: 0.156, // Average diesel car, medium size — DEFRA 2023
      hybrid: 0.11, // Petrol-electric hybrid — DEFRA 2023
      ev: 0.047, // Battery EV on average global grid — DEFRA 2023 / IEA
      default: 0.17, // Fallback if fuel type unknown
    },
    ...
    bike: 0.0, // Zero direct emissions; manufacturing amortised over years
  ...
  energy: {
    ...
    electricity_renewable: 0.02, // Certified renewable/solar — lifecycle estimate
    ...
    heating_gas_hour: 0.22, // Natural gas space heater, ~1.2 kWh/hr equivalent — DEFRA
  ...
  gridIntensityByCountry: {
    ...
    australia: { label: "🇦🇺 Australia", kg: 0.61 },
    japan: { label: "🇯🇵 Japan", kg: 0.471 },
    south_africa: { label: "🇿🇦 South Africa", kg: 0.9 },
    brazil: { label: "🇧🇷 Brazil", kg: 0.074 },
    canada: { label: "🇨🇦 Canada", kg: 0.13 },
...
export function getFoodFactor(mealType) {
  return (
    EMISSION_FACTORS.food[mealType] ?? EMISSION_FACTORS.food.meal_vegetarian
  );
}
...
export function getElectricityFactor(energyType, country = null) {
  if (energyType === "renewable")
    return EMISSION_FACTORS.energy.electricity_renewable;
  // Use country-specific grid intensity when available (for 'grid' or 'unsure')
  if (
    country &&
    country !== "world_avg" &&
    EMISSION_FACTORS.gridIntensityByCountry[country]
  ) {
