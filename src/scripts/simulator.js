 ...
 * Lets users see the annual CO₂e savings from potential behaviour changes.
 * All calculations use the same emission factors as the main logger — fully auditable.
 */
    ...
    <div class="simulator-wrapper">
      ...
      <div class="sim-baseline-card glass-card">
        <div class="sim-baseline-header">
          <div>
            <h3 class="sim-baseline-title">📊 Your estimated annual footprint</h3>
          ...
          <div class="sim-baseline-total">
            ...
            <span class="sim-big-unit">kg CO₂e / year</span>
          </div>
        </div>
        <div class="sim-cat-row">
          <div class="sim-cat-chip" style="--cat-color:#4ecdc4"><span>🚗</span><strong>${annual.transport.toFixed(0)} kg</strong><small>Transport</small></div>
          <div class="sim-cat-chip" style="--cat-color:#ffd93d"><span>🍽️</span><strong>${annual.food.toFixed(0)} kg</strong><small>Food</small></div>
          <div class="sim-cat-chip" style="--cat-color:#ff6b6b"><span>⚡</span><strong>${annual.energy.toFixed(0)} kg</strong><small>Energy</small></div>
          <div class="sim-cat-chip" style="--cat-color:#a78bfa"><span>🛍️</span><strong>${annual.consumption.toFixed(0)} kg</strong><small>Shopping</small></div>
        </div>
      </div>

      <h3 class="sim-scenarios-heading">🔮 What if you changed these habits?</h3>
      <p class="sim-scenarios-sub">Adjust each slider to see your potential annual CO₂e savings</p>
      ...
      <div class="sim-scenarios-grid">
        ...
        <div class="sim-scenario-card glass-card">
          <div class="sim-scenario-top">
            <span class="sim-scenario-emoji">🥦</span>
            <div>
              <h4 class="sim-scenario-title">Replace meat meals with plant-based</h4>
              <p class="sim-scenario-sub-text">Each beef→vegan swap saves <strong>5.6 kg CO₂e</strong> per meal</p>
            </div>
          </div>
          ...
          <div class="sim-saving-box">
            ...
            <span class="sim-saving-unit">kg CO₂e saved / year</span>
          </div>
          ...
          <div class="sim-control">
            <div class="sim-control-row-inline">
              ...
              <select id="transport-target" class="sim-select" aria-label="Target transport mode">
                <option value="ev">⚡ EV (0.047 kg/km)</option>
                <option value="bus">🚌 Bus (0.089 kg/km)</option>
                <option value="train">🚆 Train (0.035 kg/km)</option>
                <option value="bike">🚲 Cycling (0 kg/km)</option>
              </select>
            </div>
        ...
        <div class="sim-scenario-card glass-card">
          <div class="sim-scenario-top">
            <span class="sim-scenario-emoji">☀️</span>
            <div>
              <h4 class="sim-scenario-title">Switch to renewable electricity</h4>
              <p class="sim-scenario-sub-text">
                Your grid (${gridData?.label ?? 'world avg'}): <strong>${gridFactor.toFixed(3)} kg/kWh</strong> →
          ...
          <div class="sim-control">
            <div class="sim-energy-toggle" role="group" aria-label="Energy source toggle">
              <button class="sim-toggle-btn active" id="energy-grid-btn" aria-pressed="true">📍 Currently on grid</button>
              <button class="sim-toggle-btn" id="energy-renew-btn" aria-pressed="false">→ Switch to renewable</button>
            </div>
          </div>
          <div class="sim-saving-box">
        ...
        <div class="sim-scenario-card glass-card">
          <div class="sim-scenario-top">
            ...
            <div>
              ...
              <p class="sim-scenario-sub-text">Consolidating orders cuts last-mile trips (~0.5 kg CO₂e per parcel)</p>
            </div>
          </div>
          <div class="sim-control">
      ...
      <div class="sim-total-card glass-card">
        <div class="sim-total-row">
          <div>
            <h3 class="sim-total-title">🎯 Combined potential annual saving</h3>
          ...
          <div class="sim-total-right">
            ...
            <span class="sim-total-unit">kg CO₂e / year</span>
          </div>
        </div>
...
function _initSimulatorEvents(annual, energySaving) {
  ...
  function _setEquiv(id, kg) {
    ...
    el.textContent = `≈ not driving ${km} km · like ${trees} tree${trees !== 1 ? 's' : ''} absorbing for a year`;
  }
  ...
  function _updateTotal() {
    ...
    if (noteEl && total > 0) {
      ...
      noteEl.textContent = `That's a ${pct}% reduction from your estimated footprint — like planting ${trees} trees 🌳`;
    }
  }
  ...
  document.getElementById('wfh-slider')?.addEventListener('input', e => {
    ...
    // Commute ≈ 1/5 of weekly transport; cap at annual total
  ...
  // ── Transport switch slider + select ──
