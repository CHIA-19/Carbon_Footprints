...
export function sanitiseCarFuelType(fuelType) {
  const cleaned = String(fuelType || '').trim().toLowerCase();
...
export function sanitiseEnergyType(energyType) {
  const cleaned = String(energyType || '').trim().toLowerCase();
...
export function sanitiseDietPattern(diet) {
  const cleaned = String(diet || '').trim().toLowerCase();
...
export function sanitiseLogInputs(raw) {
  return {
    ...
    distanceKm:       clampNumber(raw.distanceKm,       FIELD_BOUNDS.distanceKm.min,       FIELD_BOUNDS.distanceKm.max),
    meatMeals:        clampNumber(raw.meatMeals,        FIELD_BOUNDS.meatMeals.min,        FIELD_BOUNDS.meatMeals.max),
    poultryMeals:     clampNumber(raw.poultryMeals,     FIELD_BOUNDS.poultryMeals.min,     FIELD_BOUNDS.poultryMeals.max),
    vegMeals:         clampNumber(raw.vegMeals,         FIELD_BOUNDS.vegMeals.min,         FIELD_BOUNDS.vegMeals.max),
    veganMeals:       clampNumber(raw.veganMeals,       FIELD_BOUNDS.veganMeals.min,       FIELD_BOUNDS.veganMeals.max),
    deliveryOrders:   clampNumber(raw.deliveryOrders,   FIELD_BOUNDS.deliveryOrders.min,   FIELD_BOUNDS.deliveryOrders.max),
    acHours:          clampNumber(raw.acHours,          FIELD_BOUNDS.acHours.min,          FIELD_BOUNDS.acHours.max),
    electricityKwh:   clampNumber(raw.electricityKwh,   FIELD_BOUNDS.electricityKwh.min,   FIELD_BOUNDS.electricityKwh.max),
    parcels:          clampNumber(raw.parcels,          FIELD_BOUNDS.parcels.min,          FIELD_BOUNDS.parcels.max),
    clothingItems:    clampNumber(raw.clothingItems,    FIELD_BOUNDS.clothingItems.min,    FIELD_BOUNDS.clothingItems.max),
